...
{
    public class GPTimeZoneOld
    {
        public class Rule
        {
            public int YearStart;
            public int YearEnd;
            public String RuleTextStart = "";
            public String RuleTextEnd = "";
        ...
        public class Transition
        {
            ...
            {
                GPTimestamp ts = new GPTimestamp(Timestamp);
                ts.subtractSeconds(-OffsetInSeconds);
        ...
        public string SymbolNormalTime = "";
        public string SymbolDaylightTime = "";
        ...
        public string getOffsetString()
        {
            return GPTimeZoneOld.SecondsToString(OffsetSeconds, 2);
        }
        ...
        internal int getTransCountForYear(int p)
        {
            int cnt = 0;
            foreach (Transition tr in Transitions)
            {
                if (tr.getDateTime().Year == p)
                    cnt++;
            }
            return cnt;
        }

        internal string getTimeAbbr(bool p)
        {
            foreach (Transition tr in Transitions)
            {
                if (tr.Dst == p && tr.getDateTime().Year > 2014)
        ...
        internal void RemoveTransitionsForRule(Rule rule)
        {
            for (int i = Transitions.Count - 1; i >= 0; i--)
            {
                Transition tr = Transitions[i];
                int year = tr.getDateTime().Year;
                if (year >= rule.YearStart && year <= rule.YearEnd && (rule.RuleTextEnd == tr.Rule2 || rule.RuleTextStart == tr.Rule2))
                {
